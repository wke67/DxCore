...
GCCROOT=${GCCROOT:-~/.arduino15/packages/DxCore/tools/avr-gcc/7.3.0-atmel3.6.1-azduino8a/}

rm logfile
rm config_avr*

for name in $(ls $GCCROOT/avr/include/avr* | sed -n -e 's/.*io\(avr[0-9]*[d,e][a-t].*\).h/\1/p' ); do
